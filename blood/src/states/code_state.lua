...
function CodeState.events(event, window, data)
    ...
    if event[1] == 'key' then
        ...
        elseif event[2] == keys.left then
            data.codeCursor.x, data.codeCursor.y, data.scroll = Utils.computeNewCursorPosition(data, data.codeCursor.x - 1, data.codeCursor.y)
        elseif event[2] == keys.right then
            data.codeCursor.x, data.codeCursor.y, data.scroll = Utils.computeNewCursorPosition(data, data.codeCursor.x + 1, data.codeCursor.y)
        elseif event[2] == keys.up then
            data.codeCursor.x, data.codeCursor.y, data.scroll = Utils.computeNewCursorPosition(data, data.codeCursor.x, data.codeCursor.y - 1)
